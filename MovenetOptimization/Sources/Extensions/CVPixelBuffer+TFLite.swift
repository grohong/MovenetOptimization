...
extension CVPixelBuffer {
    ...
    func addPadding(paddingWidth: Int, paddingHeight: Int) -> CVPixelBuffer? {
        let pixelBufferType = CVPixelBufferGetPixelFormatType(self)
        guard
            pixelBufferType == kCVPixelFormatType_32BGRA || pixelBufferType == kCVPixelFormatType_32ARGB
        else {
        ...
        let paddedWidth = originalWidth + paddingWidth * 2
        let paddedHeight = originalHeight + paddingHeight * 2
        ...
        guard status == kCVReturnSuccess, let buffer = paddedPixelBuffer else {
            return nil
        }
        ...
        let dstBaseAddress = CVPixelBufferGetBaseAddress(buffer)
        ...
        for row in 0..<originalHeight {
            let srcPtr = srcBaseAddress!.advanced(by: row * srcBytesPerRow)
            let dstPtr = dstBaseAddress!.advanced(by: (row + paddingHeight) * dstBytesPerRow + paddingWidth * 4)
